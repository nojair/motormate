    ...
    <div class="md:container md:mx-auto pt-12 mb-11">
      ...
      <form class="mb-4 w-11/12 mx-5 p-5 flex flex-col justify-center items-start bg-rose-100 px-4 py-5 drop-shadow-md">
        <span class="flex flex-row justify-between items-center w-full mb-6">
          ...
          <button type="button" @click="userStore.addCar" class="pointer-default hover:opacity-90 border-2 hover:border-blue-700 hover:bg-blue-100 hover:text-blue-700 text-blue-100 bg-blue-700 font-bold rounded-md px-4 py-1">Agregar automóvil <i class="fas fa-plus"></i></button>
        </span>
      ...
      <button :disabled="userStore.isSubmitting" type="button" @click="submitFormData" class="w-11/12 flex flex-row justify-center items-center mx-5 mb-3 p-1 rounded-md bg-blue-700 hover:opacity-80 text-blue-50 font-bold">{{ userStore.isSubmitting ? 'ENVIANDO...' : 'GUARDAR' }}</button>
    </div>
  </Layout>
</template>